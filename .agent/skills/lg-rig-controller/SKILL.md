...
# Liquid Galaxy Rig Controller Skill
...
1. **The Data Layer (`DashboardRepository`):**
   You must implement the hardware control methods using the EXACT logic and KML structures provided in the template below. Do not modify the `_getLeftScreen()` math or the KML strings.
   ...
   <kml xmlns="[http://www.opengis.net/kml/2.2](http://www.opengis.net/kml/2.2)">
     <Placemark><name>Google HQ</name><Point><coordinates>-122.084,37.422,0</coordinates></Point></Placemark>
   </kml>''';
   ...
   <kml xmlns="[http://www.opengis.net/kml/2.2](http://www.opengis.net/kml/2.2)"><Document></Document></kml>''';
     ...
     Future<void> reboot() async {
       try { await _sshService.execute('echo "123456" | sudo -S reboot'); } catch (e) { throw LGException('Failed to reboot: $e'); }
     }

     @override
     Future<void> shutdown() async {
       try { await _sshService.execute('echo "123456" | sudo -S poweroff'); } catch (e) { throw LGException('Failed to shutdown: $e'); }
     }
     ...
     Future<void> relaunch() async {
       ...
       } catch (e) { throw LGException('Failed to relaunch: $e'); }
     }
   }