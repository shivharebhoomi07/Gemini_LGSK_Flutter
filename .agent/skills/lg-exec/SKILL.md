⚙️ Strategic Execution Engine (lg-exec)
📋 Directive Overview
...
🚨 CRITICAL GUARDRAIL: The Anti-Zombie Rule
...
Says “Go on” without engaging with the architectural why

👉 You MUST halt execution and trigger the Skeptical Mentor
../lg-skeptical-mentor/SKILL.md
...
1️⃣ The Manifest Rule

Before planning or writing a single line of code, you MUST silently read:
...
❌ Ignoring the Manifest guarantees execution failure.

2️⃣ The Connection UI Mandate (No Hardcoding)
...
Rule:
Credentials must persist in local state or storage so they can be changed during a live demo.

3️⃣ The “Ask Gemini” (Zero-Hallucination) Protocol
...
📌 If context is still missing → halt and ask the user to paste documentation

🔌 The Liquid Galaxy Hardware Bible
(Strict Execution Patterns)
...
1️⃣ Pushing KML to Screens (Display Canvas)

To display Placemarks, Polygons, or Overlays:

Write valid KML to Apache directory on Master Node
...
KML Rule:
$kmlString must include:

XML headers

Valid <kml> wrappers

2️⃣ Moving the Camera (FlyTo / Orbit)

Camera movement does NOT use kmls.txt.

You must write a <LookAt> or <Camera> command to:
...
3️⃣ Clearing the Rig (Reset State)
...
🧭 Step 1: Blueprint Audit (Load & Review)
Read the Plan
...
Raise concerns before writing code

🧩 Step 2: Tactical Batch Execution

Rule: Execute 2–3 tasks per batch only.
...
Implementation
Logic ➔ Interface ➔ Execution (as planned)

Educational Micro-Check
Explain why this snippet exists in Clean Architecture

Tooling Verification

Run flutter analyze

If BLoC/Repository modified → flutter test

Hot Reload (r) and verify KML transmission

State Preservation (Git)
Commit using:

feat([module]): [atomic description]
...
SSH transmission confirmation

Engineering Principles Applied

Example:

Dependency Inversion via GetIt decouples UI from network logic.

🎬 Wow Factor Check

Ask:

"How does the KML execution look on the Liquid Galaxy rig? Does it achieve cinematic impact?"

Documentation Updates

Mark tasks complete in docs/plans/*.md

Append exact debrief to:
...
"Are you ready for the next batch? Does the BLoC ➔ SSH flow still make sense?"
...
Verify LG screen execution

Document Closure

Mark all tasks [x] Complete in plan
...
../lg-code-reviewer/SKILL.md

🛑 Hard Halt Conditions (Immediate Stop)
...
❌ Never guess
✅ Always halt and analyze

🔑 Core Engineering Axioms (Always Active)

Sync is Everything
If KML isn’t injected → task failed

Report the Why
Every batch must name the engineering principle used

Guardrail Activation
If the student asks:

"Just write the rest of the code"

👉 Immediately invoke Skeptical Mentor
